...
function getSelectedProductTradeMethod() {
  const checked = Array.from(document.querySelectorAll('input[name="product-trade"]:checked'))
    .map(input => input.value);

  if (checked.length >= 2) return '전국+직거래 모두';
...
function mapLegacyCategoryToId(categoryText) {
  ...
  return map[categoryText] || 7;
}

function mapLegacyBrandToId(brandText) {
  const brand = String(brandText || '').trim().toLowerCase();
...
function getCurrentUserForProduct() {
  let user = null;

  try {
    if (typeof getSafeAuthUser === 'function') {
      user = getSafeAuthUser();
  ...
  return {
    email: user?.email || localStorage.getItem('rg_email') || '',
    name: user?.nickname || user?.name || localStorage.getItem('rg_nickname') || '익명'
  };
}

async function submitProductToDB() {
...
const selectedCategoryPathLabel = document.getElementById('dropCategoryPathLabel')?.value || '';
  ...
  const tradeMethod = getSelectedProductTradeMethod();
  const images = getProductImagesFromPreview();
  const user = getCurrentUserForProduct();
  if (!user.email) {
  showToast('로그인 후 상품을 등록할 수 있습니다.', 'error');
  openModal('login');
  return;
}
  ...
  if (!name) {
    ...
    document.getElementById('dropName')?.focus();
    return;
  }

  if (!brandText) {
    showToast('브랜드를 입력해주세요.', 'error');
    document.getElementById('dropBrand')?.focus();
    return;
  }
  ...
  if (!price || price <= 0) {
    ...
    document.getElementById('dropPrice')?.focus();
    return;
  }

  if (!condition) {
    showToast('상품 상태를 선택해주세요.', 'error');
    return;
  }
...
function initProductSubmitDB() {
  const submitBtn = document.getElementById('dropSubmitBtn');
  if (!submitBtn) return;

  submitBtn.addEventListener('click', submitProductToDB);
}
